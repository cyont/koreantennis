...
$query_rsNumberCounts .= "(SELECT count(gender) FROM members M INNER JOIN groups G ON M.Id = G.Id WHERE gender = 'F' AND active_member = 1 AND inactive = 0) AS Female_Num, ";
$query_rsNumberCounts .= "(SELECT count(ethnicity_korean) FROM members M INNER JOIN groups G ON M.Id = G.Id WHERE ethnicity_korean = 1 AND active_member = 1 AND inactive = 0) AS Korean_Num, ";
...
<body id="main_body">
   ...
   <table>
       ...
       <div class="content2">
       <h3 class="contentmaintitle_green"><?php echo date("Y년 n월 j일"); ?> 현재 Active Member 분포</h3>
       <p>아래 Active Member 분포 차트를 참조하세요.</p>
       <span class="contenttitle_orange">현재 활동하는 회원 수: <?php echo $total_number; ?></span>
       <p>
...
자영업자, 샐러리맨, 주부, 한국에서 온 방문자, 그 밖의 다양한 직종 종사자 
</div>
</td>
</tr>
</table>
