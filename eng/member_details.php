...
$query_rsPersonInfo = "SELECT first_name_eng, last_name_eng, G.* FROM members M INNER JOIN groups G ON M.Id = G.Id ";
...
<body id="main_body">
   ...
   <table>
       ...
       <h3><span class="contentsupertitle_orange"><?php echo $row_rsPersonInfo['first_name_eng'] . " " . $row_rsPersonInfo['last_name_eng']; ?></span> Game Details</h3> 
  <?php
  ...
  <?php if ($total_current_matches == $total_total_matches) { ?>
  	<span class="contenttitle_green">Game Score: </span><?php echo $row_rsPersonInfo['current_wins']; ?> Win<?php if ($row_rsPersonInfo['current_wins'] > 1) { echo "s"; } ?> <?php echo $row_rsPersonInfo['current_losses']; ?> Loss<?php if ($row_rsPersonInfo['current_losses'] > 1) { echo "es"; } ?><br />
    <span class="contenttitle_green">Winning Percentage: </span><?php echo $winning_percentage; ?> %
  
  <!--If the current group records and total group records are different -->
  <?php } else { ?>
  	<span class="contenttitle_green">Current Group Game Score: </span><?php echo $row_rsPersonInfo['current_wins']; ?> Win<?php if ($row_rsPersonInfo['current_wins'] > 1) { echo "s"; } ?> <?php echo $row_rsPersonInfo['current_losses']; ?> Loss<?php if ($row_rsPersonInfo['current_losses'] > 1) { echo "es"; } ?><br />
    <span class="contenttitle_green">Current Group Winning Percentage: </span><?php echo $winning_percentage; ?> %<br />
  	<span class="contenttitle_green">Total Game Score: </span><?php echo $row_rsPersonInfo['total_wins']; ?> Win<?php if ($row_rsPersonInfo['total_wins'] > 1) { echo "s"; } ?> <?php echo $row_rsPersonInfo['total_losses']; ?> Loss<?php if ($row_rsPersonInfo['total_losses'] > 1) { echo "es"; } ?><br />
    <span class="contenttitle_green">Total Winning Percentage: </span><?php echo $total_winning_percentage; ?> %<br />
  <?php } ?>
  </p>
...
<?php if ($totalRows_rsMemberDetails > 0) { ?>
  ...
  <tr align="center" bgcolor="#CCCCCC">
    <td width="12%">Game Date</td>
    <td width="21%">Person A</td>
    <td width="16%">Person A Score</td>
    <td width="17%">Person B Score</td>
  ...
  <?php do { $current_row++ ?>
	...
	$query_rsNames = "SELECT first_name_eng, last_name_eng FROM members WHERE Id = '" . $row_rsMemberDetails['person1'] . "'";
	...
	<?php echo $row_rsNames['first_name_eng'] . " " . $row_rsNames['last_name_eng']; ?>     </td>
	...
	$query_rsNames2 = "SELECT first_name_eng, last_name_eng FROM members WHERE Id = '" . $row_rsMemberDetails['person2'] . "'";
	...
	<?php echo $row_rsNames2['first_name_eng'] . " " . $row_rsNames2['last_name_eng']; 
  ?>      </td>
      <td>
