...
members_add.Form_CustomValidate =  
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $members->TableCaption() ?><br><br>
...
<?php if ($members->gender->Visible) { // gender ?>
	<tr<?php echo $members->RowAttributes() ?>>
        ...
        <input name="x_gender" type="radio" value="M" checked /> Male &nbsp;&nbsp;&nbsp;
        <input name="x_gender" type="radio" value="F" /> Female        
...
<?php if ($members->status_student->Visible) { // status_student ?>
	<tr<?php echo $members->RowAttributes() ?>>
        ...
        <input name="x_status_student" type="radio" value="1" /> Student &nbsp;&nbsp;&nbsp;
        <input name="x_status_student" type="radio" value="0" /> Non-Student
...
<?php if ($members->ethnicity_korean->Visible) { // ethnicity_korean ?>
	<tr<?php echo $members->RowAttributes() ?>>
        ...
        <input name="x_ethnicity_korean" type="radio" value="1" checked="checked" /> Korean &nbsp;&nbsp;&nbsp;
        <input name="x_ethnicity_korean" type="radio" value="0" /> Non-Korean
...
<?php if ($members->language_korean->Visible) { // language_korean ?>
	<tr<?php echo $members->RowAttributes() ?>>
        ...
        <input name="x_language_korean" type="radio" value="1" checked="checked" /> Korean Language&nbsp;&nbsp;&nbsp;
        <input name="x_language_korean" type="radio" value="0" /> Non-Korean or Bilingual
...
<?php if ($members->access_level->Visible) { // access_level ?>
	<tr<?php echo $members->RowAttributes() ?>>
        ...
        <input name="x_access_level" type="radio" value="1" checked="checked" /> General&nbsp;&nbsp;&nbsp;
        <input name="x_access_level" type="radio" value="2" /> Administrator&nbsp;&nbsp;&nbsp;
        <input name="x_access_level" type="radio" value="3" /> Database Owner
...
<?php if ($members->active_member->Visible) { // active_member ?>
	<tr<?php echo $members->RowAttributes() ?>>
        ...
        <input name="x_active_member" type="radio" value="1" checked="checked" /> Yes&nbsp;&nbsp;&nbsp;
        <input name="x_active_member" type="radio" value="0" /> No
...
class cmembers_add {
	...
	function LoadDefaultValues() {
		global $members;
		$members->access_level->CurrentValue = 1;
		$members->active_member->CurrentValue = 1;
	}
	...
	function AddRow() {
		...
		// access_level
		$members->access_level->SetDbValueDef($rsnew, $members->access_level->CurrentValue, NULL, TRUE);

		// active_member
		$members->active_member->SetDbValueDef($rsnew, $members->active_member->CurrentValue, NULL, TRUE);

		// Call Row Inserting event
		$bInsertRow = $members->Row_Inserting($rsnew);
		if ($bInsertRow) {
		...
		if ($AddRow) {
			...
			$rsnew['Id'] = $members->Id->DbValue;

			$members->Row_Inserted($rsnew);

		// After the insertion, get the lastest ID.
		require_once('../Connections/connTennis.php'); 
		mysql_select_db($database_connTennis, $connTennis);
		$latestID = "SELECT max(Id) AS Id FROM members";
		$rsMaxID = mysql_query($latestID, $connTennis) or die(mysql_error());
		$row_rsMaxID = mysql_fetch_assoc($rsMaxID);
		$max_id = $row_rsMaxID['Id'];
				
		// Insert the lastest ID into the Groups table.
		$insertSQL = "INSERT INTO groups (Id) VALUES ($max_id) ";
		$Result1 = mysql_query($insertSQL, $connTennis) or die(mysql_error());

			// Send Welcome email.
			$zemail = $members->first_name->CurrentValue = $members->zemail->FormValue;
				if ((isset($zemail))  && (strlen($zemail) > 0)) {
				$mailto =  "$zemail";
				$subject =  "Tucson Korean Tennis: Welcome";
				$mailbody =  "Welcome to Tucson Korean Tennis Association! \n";
				$mailbody .=  "You've registered as a new member.  Please check our website, http://koreantennis.org, for more information, announcements, tennis tournaments and member benefits from local sponsors. You can send email at contact@koreantennis.org for questions and comments. \n\n";
				$mailbody .=  "Tucson Korean Tennis Association \n";
