...
// Create page object
$courts_edit = new ccourts_edit();
$Page =& $courts_edit;

// Page init
$courts_edit->Page_Init();

// Page main
$courts_edit->Page_Main();
?>
...
// Create page object
var courts_edit = new ew_Page("courts_edit");

// page properties
courts_edit.PageID = "edit"; // page ID
courts_edit.FormID = "fcourtsedit"; // form ID
var EW_PAGE_ID = courts_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
courts_edit.ValidateForm = function(fobj) {
...
courts_edit.Form_CustomValidate =  
...
courts_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
courts_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $courts->TableCaption() ?><br><br>
<a href="<?php echo $courts->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$courts_edit->ShowMessage();
?>
<form name="fcourtsedit" id="fcourtsedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return courts_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="courts">
...
<?php if ($courts->id->Visible) { // id ?>
	<tr<?php echo $courts->RowAttributes() ?>>
		...
		<td<?php echo $courts->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $courts->id->ViewAttributes() ?>><?php echo $courts->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($courts->id->CurrentValue) ?>">
</span><?php echo $courts->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
class ccourts_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$courts->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$courts->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$courts->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$courts->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($courts->id->CurrentValue == "")
			$this->Page_Terminate("courtslist.php"); // Invalid key, return to list
		switch ($courts->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$courts->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$courts->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($courts->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id
			$courts->id->EditCustomAttributes = "";
			$courts->id->EditValue = $courts->id->CurrentValue;
			$courts->id->CssStyle = "";
			$courts->id->CssClass = "";
			$courts->id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $courts->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($courts->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$courts->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
