...
class cmembers {
	...
	function SqlSelect() { // Select
		return "SELECT M.* FROM members M INNER JOIN groups G ON M.Id = G.Id ";
	}

	function SqlWhere() { // Where
		$sWhere = "";
		$this->TableFilter = "active_member = 1 AND length(email) > 0 AND r_group = 'A' AND inactive = 0";
		if ($this->TableFilter <> "") {
	...
	function LoadListRowValues(&$rs) {
		$this->Id->setDbValue($rs->fields('M.Id'));
