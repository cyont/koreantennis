...
// Create page object
$groups_add = new cgroups_add();
$Page =& $groups_add;

// Page init
$groups_add->Page_Init();

// Page main
$groups_add->Page_Main();
?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $groups->TableCaption() ?><br><br>
...
?>
<form name="fgroupsadd" id="fgroupsadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return groups_add.ValidateForm(this);">
<p>
...
<input type="text" name="x_Id" id="x_Id" title="<?php echo $groups->Id->FldTitle() ?>" size="30" value="<?php echo $groups->Id->EditValue ?>"<?php echo $groups->Id->EditAttributes() ?>>
...
class cgroups_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $groups->CurrentAction = $_POST["a_add"]; // Get form action
	...
	function LoadDefaultValues() {
		global $groups;
		$groups->Id->CurrentValue = 0;
		$groups->inactive->CurrentValue = 0;
		$groups->points->CurrentValue = 1000;
		$groups->hide_list->CurrentValue = 0;
		$groups->total_wins->CurrentValue = 0;
		$groups->total_losses->CurrentValue = 0;
		$groups->current_wins->CurrentValue = 0;
		$groups->current_losses->CurrentValue = 0;
	}
	...
	function RenderRow() {
		...
		} elseif ($groups->RowType == EW_ROWTYPE_ADD) { // Add row

			// Id
			$groups->Id->EditCustomAttributes = "";
			$groups->Id->EditValue = ew_HtmlEncode($groups->Id->CurrentValue);
	...
	function AddRow() {
		...
		if ($groups->Id->CurrentValue == "" && $groups->Id->getSessionValue() == "") {
		...
		// Id
		$groups->Id->SetDbValueDef($rsnew, $groups->Id->CurrentValue, 0, TRUE);
		...
		// inactive
		$groups->inactive->SetDbValueDef($rsnew, $groups->inactive->CurrentValue, NULL, TRUE);

		// r_group
		$groups->r_group->SetDbValueDef($rsnew, $groups->r_group->CurrentValue, NULL, FALSE);

		// points
		$groups->points->SetDbValueDef($rsnew, $groups->points->CurrentValue, NULL, TRUE);

		// hide_list
		$groups->hide_list->SetDbValueDef($rsnew, $groups->hide_list->CurrentValue, NULL, TRUE);

		// total_wins
		$groups->total_wins->SetDbValueDef($rsnew, $groups->total_wins->CurrentValue, NULL, TRUE);

		// total_losses
		$groups->total_losses->SetDbValueDef($rsnew, $groups->total_losses->CurrentValue, NULL, TRUE);

		// current_wins
		$groups->current_wins->SetDbValueDef($rsnew, $groups->current_wins->CurrentValue, NULL, TRUE);

		// current_losses
		$groups->current_losses->SetDbValueDef($rsnew, $groups->current_losses->CurrentValue, NULL, TRUE);

		// Call Row Inserting event
		$bInsertRow = $groups->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($groups->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
