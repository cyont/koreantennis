...
// Create page object
var pics_add = new ew_Page("pics_add");

// page properties
pics_add.PageID = "add"; // page ID
pics_add.FormID = "fpicsadd"; // form ID
var EW_PAGE_ID = pics_add.PageID; // for backward compatibility

// extend page with ValidateForm function
pics_add.ValidateForm = function(fobj) {
...
pics_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
pics_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $pics->TableCaption() ?><br><br>
<a href="<?php echo $pics->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$pics_add->ShowMessage();
?>
<form name="fpicsadd" id="fpicsadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return pics_add.ValidateForm(this);">
<p>
...
class cpics_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $pics->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$pics->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $pics->CurrentAction = "C"; // Copy record
		  } else {
		    $pics->CurrentAction = "I"; // Display blank record
		...
		switch ($pics->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$pics->SendEmail = TRUE; // Send email on add success
		...
		$pics->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function RenderRow() {
		...
		} elseif ($pics->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		// hits
		$pics->hits->SetDbValueDef($rsnew, $pics->hits->CurrentValue, 0, TRUE);

		// date_uploaded
		$pics->date_uploaded->SetDbValueDef($rsnew, ew_UnFormatDateTime($pics->date_uploaded->CurrentValue, 6, FALSE), ew_CurrentDate());

		// Call Row Inserting event
		$bInsertRow = $pics->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($pics->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$pics->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] = $pics->id->DbValue;

			// Call Row Inserted event
			$pics->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
