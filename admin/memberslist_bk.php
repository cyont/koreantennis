...
<?php if ($members->active_member->Visible) { // active_member ?>
	<?php if ($members->SortUrl($members->active_member) == "") { ?>
		<td><?php echo $members->active_member->FldCaption() ?></td>
	<?php } else { ?>
		<td><div class="ewPointer" onmousedown="ew_Sort(event,'<?php echo $members->SortUrl($members->active_member) ?>',1);">
			<table cellspacing="0" class="ewTableHeaderBtn"><thead><tr><td><?php echo $members->active_member->FldCaption() ?></td><td style="width: 10px;"><?php if ($members->active_member->getSort() == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif ($members->active_member->getSort() == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></td></tr></thead></table>
		</div></td>		
	<?php } ?>
<?php } ?>		
...
<div<?php echo $members->active_member->ViewAttributes() ?>><?php echo $members->active_member->ListViewValue() ?></div>
</td>
	<?php } ?>
	</tr>
<?php
...
class cmembers_list {
	...
	function Page_Main() {
		...
		if ($members->getRecordsPerPage() <> "") {
			...
			$this->lDisplayRecs = 100; // Load default
		}
